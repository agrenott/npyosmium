...
def test_file_processor_bad_init(init):
    with pytest.raises(TypeError):
        for obj in npyosmium.FileProcessor(init):
            pass


def test_simple_generator(opl_buffer):
    count = 0
    for obj in npyosmium.FileProcessor(opl_buffer('n1 x5 y5')):
...
def test_generator_with_location(opl_buffer):
    ...
    count = 0
    for obj in npyosmium.FileProcessor(data).with_locations():
        count += 1
        if obj.type_str() == 'w':
...
def test_generator_with_areas(opl_buffer):
    ...
    count = 0
    for obj in npyosmium.FileProcessor(data).with_areas():
        if obj.type_str() == 'a':
            count += 1
            assert obj.from_way()
...
def test_generator_with_areas_with_filter(opl_buffer):
    ...
    count = 0
    for obj in npyosmium.FileProcessor(data)\
                     .with_areas()\
                     .with_filter(npyosmium.filter.EntityFilter(npyosmium.osm.AREA)):
        assert obj.is_area()
        count += 1
...
def test_generator_with_areas_with_area_filter(opl_buffer):
    ...
    count = 0
    for obj in npyosmium.FileProcessor(data)\
                     .with_areas(npyosmium.filter.KeyFilter('building'))\
                     .with_filter(npyosmium.filter.EntityFilter(npyosmium.osm.AREA)):
        assert obj.is_area()
        assert obj.id == 3
        count += 1
...
def test_generator_with_filter(opl_buffer):
    ...
    count = 0
    for obj in npyosmium.FileProcessor(data).with_filter(npyosmium.filter.EmptyTagFilter()):
        count += 1
        assert obj.type_str() == 'n'
        assert obj.id == 11
...
def test_file_processor_header(tmp_path):
    ...
    h = npyosmium.FileProcessor(fn).header
...
def test_file_processor_access_nodestore(opl_buffer):
    fp = npyosmium.FileProcessor(opl_buffer('n56 x3 y-3'))\
          .with_locations(npyosmium.index.create_map('sparse_mem_map'))
...
def test_file_processor_bad_location_type(opl_buffer):
    with pytest.raises(TypeError, match='LocationTable'):
        npyosmium.FileProcessor(opl_buffer('n56 x3 y-3')).with_locations(67)
...
def test_propagate_data_from_filters(opl_buffer):
    ...
    fp = npyosmium.FileProcessor(opl_buffer('n56 x3 y-3')).with_filter(MyFilter())
...
def test_simple_zip(opl_buffer):
    fp1 = npyosmium.FileProcessor(opl_buffer("""\
            n1
            n3
            n5
            w10 Nn1,n2
    ...
    fp2 = npyosmium.FileProcessor(opl_buffer("""\
            n2
            n3
            n456
            w12 Nn45,n85
    ...
    results = []
    for o1, o2 in npyosmium.zip_processors(fp1, fp2):
...
def test_filtered_handler_python(opl_buffer):
    ...
    fp = npyosmium.FileProcessor(data)\
               .handler_for_filtered(ids)\
               .with_filter(npyosmium.filter.EmptyTagFilter())
...
def test_filtered_handler_basehandler(opl_buffer, tmp_path):
    ...
    with npyosmium.SimpleWriter(str(testf)) as writer:
        fp = npyosmium.FileProcessor(data)\
