...
def node_geom(test_data):
    ...
    def _run(factory, data='n1 x-23.3 y28.0'):
        ...
        handler = npyosmium.make_simple_handler(node=_mk_point)
...
def test_wkb_create_node(node_geom):
    wkb = node_geom(npyosmium.geom.WKBFactory())
...
def test_wkt_create_node(node_geom):
    wkt = node_geom(npyosmium.geom.WKTFactory())
    assert wkt.startswith('POINT(')


def test_geojson_create_node(node_geom):
    geom = node_geom(npyosmium.geom.GeoJSONFactory())
    ...
    def _run(factory):
        ...
        def _mk_way(w):
            ...
            geoms.append(factory.create_linestring(
                w, direction=npyosmium.geom.direction.BACKWARD))
            geoms.append(factory.create_linestring(
                w, use_nodes=npyosmium.geom.use_nodes.ALL))

        handler = npyosmium.make_simple_handler(way=_mk_way)
...
def test_wkb_create_way(way_geom):
    wkbs = way_geom(npyosmium.geom.WKBFactory())
...
def area_geom(test_data):
    ...
    def _run(factory):
        ...
        handler = npyosmium.make_simple_handler(area=_mk_area)
        handler.apply_file(opl, locations=True)
...
def test_wkb_create_poly(area_geom):
    wkb = area_geom(npyosmium.geom.WKBFactory())
    if wkb.startswith('01'):
...
def test_wkt_create_poly(area_geom):
    wkt = area_geom(npyosmium.geom.WKTFactory())
    assert wkt.startswith('MULTIPOLYGON(')


def test_geojson_create_poly(area_geom):
    geom = area_geom(npyosmium.geom.GeoJSONFactory())
    geom = json.loads(geom)
    assert geom['type'] == 'MultiPolygon'


def test_lonlat_to_mercator():
    c = npyosmium.geom.lonlat_to_mercator(npyosmium.geom.Coordinates(3.4, -7.3))
    assert c.x == pytest.approx(378486.2686971)
    assert c.y == pytest.approx(-814839.8325696)


def test_mercator_lonlat():
    c = npyosmium.geom.mercator_to_lonlat(npyosmium.geom.Coordinates(0.03, 10.2))
...
def test_coordinate_from_location():
    c = npyosmium.geom.Coordinates(npyosmium.osm.Location(10.0, -3.0))
...
def test_haversine():
    ...
    def call_haversine(w):
        results.append(npyosmium.geom.haversine_distance(w.nodes))

    handler = npyosmium.make_simple_handler(way=call_haversine)
...
def test_haversine_invalid_object():
    ...
    with pytest.raises(RuntimeError, match="removed OSM object"):
        npyosmium.geom.haversine_distance(results[0])


def test_haversine_coordinates():
    dist = npyosmium.geom.haversine_distance(npyosmium.geom.Coordinates(0, 0),
                                          npyosmium.geom.Coordinates(1, 1))
    assert dist == pytest.approx(157293.74877)


def test_haversine_location():
    dist = npyosmium.geom.haversine_distance(npyosmium.osm.Location(0, 0),
                                          npyosmium.osm.Location(1, 1))
    assert dist == pytest.approx(157293.74877)
