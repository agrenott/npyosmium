...
def test_broken_timestamp(test_data):
    ...
    with npyosmium.io.Reader(fn) as rd:
        with pytest.raises(RuntimeError):
            npyosmium.apply(rd, CountingHandler())
...
def test_reader_with_separate_thread_pool(test_data):
    with npyosmium.io.Reader(test_data('n1 x1 y1'), thread_pool=npyosmium.io.ThreadPool()) as rd:
        for obj in npyosmium.OsmFileIterator(rd):
            assert obj.id == 1


@pytest.mark.parametrize("entities,expected", [(npyosmium.osm.NODE, [2, 0, 0, 0]),
                                               (npyosmium.osm.ALL, [2, 1, 1, 0])])
def test_reader_with_entity_filter(test_data, entities, expected):
    ...
    h = CountingHandler()
    with npyosmium.io.Reader(fn, entities) as rd:
        npyosmium.apply(rd, h)

    assert h.counts == expected


def test_thread_pool():
    pool = npyosmium.io.ThreadPool(2, 15)
