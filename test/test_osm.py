...
def apply_with_merge(handler, data, locations, tmp_path):
    mir = npyosmium.MergeInputReader()
...
def _make_importer_factory(apply_func, tmp_path, to_opl):
    ...
    def _run(data, node=None, way=None, relation=None, area=None, changeset=None, locations=False):
        ...
        handler = npyosmium.make_simple_handler(node=_m(node), way=_m(way),
...
def test_valid_location():
    loc = npyosmium.osm.Location(-1, 10)
...
def test_object_attribute_do_not_overwrite(opl_buffer, attrname, osmdata):
    for n in npyosmium.FileProcessor(opl_buffer(osmdata)):
...
def test_node_attributes(opl_buffer):
    ...
    for n in npyosmium.FileProcessor(opl_buffer(node_data)):
...
def test_node_location(opl_buffer):
    for n in npyosmium.FileProcessor(opl_buffer("n1 x4 y5")):
        assert n.lat == 5.0
        assert n.lon == 4.0
...
def test_node_positive_id(opl_buffer, nid):
    for n in npyosmium.FileProcessor(opl_buffer(f"n{nid} v5 c58674")):
        assert n.id == nid
...
def test_way_attribute_do_not_overwrite(opl_buffer):
    data = """\
           w34 Nn34
           """
    for w in npyosmium.FileProcessor(opl_buffer(data)):
        with pytest.raises(AttributeError):
            w.nodes = [3, 4, 5]
...
def test_relation_attribute_do_not_overwrite(opl_buffer):
    data = """\
           r34 Mn23@,w34@
           """
    for r in npyosmium.FileProcessor(opl_buffer(data)):
        with pytest.raises(AttributeError):
            r.members = [3, 4, 5]
...
def test_entity_operations():
    assert not npyosmium.osm.NOTHING
    assert npyosmium.osm.NODE

    assert npyosmium.osm.AREA | npyosmium.osm.NODE | npyosmium.osm.WAY | npyosmium.osm.RELATION \
           == npyosmium.osm.OBJECT
    assert npyosmium.osm.ALL & npyosmium.osm.RELATION == npyosmium.osm.RELATION

    assert ~npyosmium.osm.CHANGESET == npyosmium.osm.OBJECT
