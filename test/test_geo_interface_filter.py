...
def test_node_geometry(opl_buffer):
    fp = npyosmium.FileProcessor(opl_buffer("n23 x3 y4"))\
          .with_filter(npyosmium.filter.GeoInterfaceFilter())
...
def test_way_geometry(opl_buffer):
    ...
    fp = npyosmium.FileProcessor(opl_buffer(data))\
               .with_locations()\
               .with_filter(npyosmium.filter.EntityFilter(npyosmium.osm.WAY))\
               .with_filter(npyosmium.filter.GeoInterfaceFilter())
...
def test_area_geometry_without_drop(opl_reader):
    ...
    ids = IDCollector()
    npyosmium.apply(opl_reader(data), npyosmium.filter.GeoInterfaceFilter(), ids)

    assert ids.nodes == [1, 2]

    ids = IDCollector()
    npyosmium.apply(opl_reader(data),
                 npyosmium.filter.GeoInterfaceFilter(drop_invalid_geometries=False),
                 ids)
...
def test_property_tag_filter(opl_buffer):
    ...
    fp = npyosmium.FileProcessor(opl_buffer(data))\
               .with_filter(npyosmium.filter.GeoInterfaceFilter(tags=['a', 'b']))
