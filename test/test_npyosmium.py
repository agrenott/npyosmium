...
def test_read_node_location_with_handler(opl_reader):
    ...
    hdlr = npyosmium.NodeLocationsForWays(idx)
    ...
    npyosmium.apply(opl_reader(data), hdlr)
...
def test_apply_node_location_handler(opl_reader, ignore_error):
    ...
    hdlr = npyosmium.NodeLocationsForWays(npyosmium.index.create_map("flex_mem"))
    if ignore_error:
        hdlr.ignore_errors()
    ...
    if ignore_error:
        npyosmium.apply(opl_reader(data), hdlr, tester)
...
def test_apply_invalid_handler_object(opl_reader):
    ...
    with pytest.raises(TypeError):
        npyosmium.apply(opl_reader("n1 x2 z4"), DummyHandler())


def test_mixed_handlers(opl_reader):
    logged = []

    class OldStyle(npyosmium.SimpleHandler):
        def node(self, n):
            logged.append('old')
    ...
    npyosmium.apply(opl_reader("n1 x0 y0"), NewStyle(), OldStyle(), NewStyle(), OldStyle())
...
def test_value_propagation(opl_reader):
    ...
    npyosmium.apply(opl_reader("n1 x0 y0"), FirstHandler(), SecondHandler())
