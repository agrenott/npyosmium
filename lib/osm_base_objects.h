    ...
    pybind11::object get_or_create_python_object()
    {
        if (!m_valid) {
            ...
            if constexpr (std::is_same_v<T, osmium::Node>) {
                m_pyobj = pybind11::module_::import("npyosmium.osm.types").attr("Node")(COSMNode{m_obj});
            } else if constexpr (std::is_same_v<T, osmium::Way>) {
                m_pyobj = pybind11::module_::import("npyosmium.osm.types").attr("Way")(COSMWay{m_obj});
            } else if constexpr (std::is_same_v<T, osmium::Relation>) {
                m_pyobj = pybind11::module_::import("npyosmium.osm.types").attr("Relation")(COSMRelation{m_obj});
            } else if constexpr (std::is_same_v<T, osmium::Area>) {
                m_pyobj = pybind11::module_::import("npyosmium.osm.types").attr("Area")(COSMArea{m_obj});
            } else if constexpr (std::is_same_v<T, osmium::Changeset>) {
                m_pyobj = pybind11::module_::import("npyosmium.osm.types").attr("Changeset")(COSMChangeset{m_obj});
            }
        }
